...
def plot_diffs():
    ...
    for it, b in enumerate(_adversary.true_memberships):
        if b == 1:
            diffs_members.append(_adversary.diffs[it])
        else:
            diffs_non_members.append(_adversary.diffs[it])

    plt.figure(figsize=(12, 8))
    plt.hist(diffs_members, color='blue', label='Members', alpha=0.5, bins=85)
    plt.hist(diffs_non_members, color='red', label='Non-members', alpha=0.5, bins=85)
    plt.legend()
...
def domias(adversary_knowledge=1.0, plot_roc=False):
    ...
    if plot_roc:
        ...
        show_roc_curve(tprs, fprs, f"Augmentation and adversary knowledge: {adversary_knowledge}", low_fprs=True)
    return tprs, fprs
